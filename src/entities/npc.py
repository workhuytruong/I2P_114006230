...
class NPC(Entity):
    ...
    def __init__(
        ...
        if facing is None:
            raise ValueError("NPC requires a 'facing' Direction at instantiation")
    ...
    def _get_los_rect(self) -> pygame.Rect | None:
        ...
        if self.los_direction == Direction.UP:
            return pygame.Rect(
                x, y - tile_size * self.max_tiles,
                tile_size, tile_size * self.max_tiles
            )
            ...
            return pygame.Rect(
                x - tile_size * self.max_tiles, y,
                tile_size * self.max_tiles, tile_size
            )
    ...
    def from_dict(cls, data: dict, game_manager: GameManager) -> "NPC":
        classification = NPCClassification(data["classification"])
